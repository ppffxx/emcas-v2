...
public class UsuarioServicioImpl implements UsuarioServicio {
    ...
    public UsuarioDTO crearUsuario(UsuarioSinIdDTO usuarioSinIdDTO) {
        ...
        } catch(RuntimeException ex) {throw new RuntimeException(Messages.getGenMessage());}

    }

    @Override
    public UsuarioReservaDTO obtenerUsuario(Integer idUsuario) {
        Usuario user = usuarioRepositorio.findById(idUsuario).orElseThrow(() -> new NotFoundException(Messages.getMessage("Usuario", idUsuario)));
        return usuarioMapper.UsuarioEntityToUsuarioReservaDTO(user);
    }

    @Override
    public void eliminarUsuario(Integer idUsuario) {
        usuarioRepositorio.findById(idUsuario).orElseThrow(() -> new NotFoundException(Messages.getMessage("Usuario", idUsuario)));
        try {usuarioRepositorio.deleteById(idUsuario);} catch(RuntimeException ex) {throw new RuntimeException(Messages.getGenMessage());}
    }
